...
func (p proximalDCAlgorithmWithExtrapolation) PDCA() (*mat64.Vector, int) {
	...
	diff.SubVec(xk, p.xk)
	if mat64.Norm(diff, 2)/math.Max(1.0, mat64.Norm(p.xk, 2)) < 1e-5 || p.iter > StopPDCA {
		return xk, p.iter
	}
	p.xkOld, p.xk, p.yk = p.xk, xk, yk
	p.iter++
	return p.PDCA()
}
...
func (p *proximalDCAlgorithmWithExtrapolation) thetaRestart() {
	...
	switch {
	case mat64.Dot(yk, xk) > 0 || p.iter == p.iterTheta:
		p.theta, p.thetaOld = 1.0, 1.0
	case p.iter == p.iterTheta:
		p.iterTheta += 200
	}
}
